...
export default function MapBoxDetails(props: Readonly<MapBoxProps>) {
    ...
    function fetchMapBoxConfig() {
        axios.get("/api/mbox/72c81498-f6b2-4a8a-911c-cd217a65e0da")
            .then((response) => {
    ...
    useEffect(() => {
        if (!props.realEstateAddress || !mapboxConfig) return; // Warten, bis Adresse und Token verfügbar sind

        const geocodeUrl = `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(
            props.realEstateAddress
    ...
    }, [props.realEstateAddress, mapboxConfig]); // `mapboxConfig` als Abhängigkeit, da es jetzt vom API-Aufruf kommt
